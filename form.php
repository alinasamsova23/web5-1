<div class="row justify-content-md-center p-2">
    ...
    <div class="form-group">
      <input type="name" placeholder="Имя" name="name" class="form-control 
      <?php if($errors['name']){echo 'border border-danger';} ?>" value="
      <?php echo $values['name']; ?>"><br>
      <input type="email" placeholder="Email" name="email" class="form-control 
      <?php if($errors['email']){echo 'border border-danger';} ?>" value="
      <?php echo $values['email']; ?>"><br>
      <input type="number" placeholder="Год рождения" name="year" class="form-control 
      <?php if($errors['year']){echo 'border border-danger';} ?>" value="
      ...
      <label class="form-check-lable" for="gender">
        ...
        <?php if(!$values['gender']){echo 'checked';}?>>
        М.
      </label>
      </div>
      <div class="form-check">
      ...
      <select multiple name="powers[]" class="form-control">
       <?php 
            $powers = unserialize($values['powers']);
            if(!$powers) $powers = array();
       ?>
        <option value=1 
        <?php if(in_array(1, $powers)) echo 'selected'; ?>>Бессмертие</option>
        <option value=2 
        <?php if(in_array(2, $powers)) echo 'selected'; ?>>Прохождение сквозь стены</option>
        <option value=3 
        <?php if(in_array(3, $powers)) echo 'selected'; ?>>Левитация</option>
      </select>
      <br>
      </div>
      <h6>Биография</h6>
      ...
      <?php if($errors['bio']){echo 'border border-danger';} ?>">
      <?php echo $values['bio']; ?></textarea>
      <br>
      <div class="form-check">
