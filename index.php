...
$dbuser = 'u20233';
$dbpass = '1488849';
$db = new PDO('mysql:host=localhost;dbname=u20233', $dbuser, $dbpass, array(PDO::ATTR_PERSISTENT => true));
?>
<!DOCTYPE html>
<html lang="ru">
...
$hash = md5($p);
try {
$stmt = $db->prepare("INSERT INTO application (login, password, name, year, powers, bio, gender, email, bodyparts) VALUES (?,?,?, ?, ?, ?, ?, ?, ?)");
...
$hash = md5($p);
try {
$stmt = $db->prepare("UPDATE application SET name=?, year=?, powers=?, bio=?, gender=?, email=?, bodyparts=? WHERE login=? AND password=?");
